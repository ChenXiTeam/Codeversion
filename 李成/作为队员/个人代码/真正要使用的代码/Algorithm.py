...
在每个的后面都有一个主要的函数说明
例如：
get_simhash_similarity(database)
（2）如果输入是一个地址（文件夹路径）
...
def get_simhash_similarity(database):
    ...
    #     sql_select = 'select * from similar where file_id = \"'+filename+'\"'
    #     if mysql.query(sql_select):
    #         filename2 = os.path.basename(filelist[dict_result[i][0]])
    #         sql2 = 'update similar set simhash_fid =\"'+filename2+\
    #                '\" , simhash_number = '+str(dict_result[i][1])+' where file_id = '+'\"'+filename+'\"'
    #         mysql.update_db(sql2)
    #     else:
    #         filename2 = os.path.basename(filelist[dict_result[i][0]])
    #         sql2 = 'insert into similar(file_id,simhash_fid,simhash_number) values(\"'+filename+'\",\"'+filename2+'\",'+str(dict_result[i][1])+')'
    #         mysql.insert_db(sql2)
...
def get_TFIDF_cosine(database):
    ...
    # 加入数据库
    # for i in range(len(result_dict2)):
    #     filename = os.path.basename(filelist[i])
    #     sql_select = 'select * from similar where file_id = \"'+filename+'\"'
    #     if mysql.query(sql_select):
    #         filename2 = os.path.basename(filelist[result_dict2[i][0]])
    #         sql2 = 'update similar set TFIDF_cosine_fid = \"'+filename2+\
    #                '\", TFIDF_cosine_number = '+str(result_dict2[i][1])+' where file_id = '+'\"'+filename+'\"'
    #         mysql.update_db(sql2)
    #     else:
    #         filename2 = os.path.basename(filelist[result_dict2[i][0]])
    #         sql2 = 'insert into similar(file_id,TFIDF_cosine_fid,TFIDF_cosine_number) values(\"'+filename+'\",\"'+filename2+'\",'+str(result_dict2[i][1])+')'
    #         mysql.insert_db(sql2)
...
def get_SparseMS(database):
    ...
    # 加入数据库
    # for i in range(len(resdict_SparseMS)):
    #     filename = os.path.basename(filelist[i])
    #     sql_select = 'select * from similar where file_id = \"'+filename+'\"'
    #     if mysql.query(sql_select):
    #         filename2 = os.path.basename(filelist[resdict_SparseMS[i][0]])
    #         sql2 = 'update similar set SparseMS_fid = \"'+filename2+\
    #                '\", SparseMS_number = '+str(resdict_SparseMS[i][1])+' where file_id = '+'\"'+filename+'\"'
    #         mysql.update_db(sql2)
    #     else:
    #         filename2 = os.path.basename(filelist[resdict_SparseMS[i][0]])
    #         sql2 = 'insert into similar(file_id,SparseMS_fid,SparseMS_number) values(\"'+filename+'\",\"'+filename2+'\",'+str(resdict_SparseMS[i][1])+')'
    #         mysql.insert_db(sql2)
...
def getKgramHashim(K,WindowSize,Base,txt1,txt2):
    ...
    similar = round(2.0*getSimnumber(dict1,dict2,1)/sum,5)
...
def get_KgramHashim(K,WindowSize,Base,database):
    ...
    # 加入数据库
    # for i in range(len(dict_result2)):
    #     filename = os.path.basename(filelist[i])
    #     sql_select = 'select * from similar where file_id = \"'+filename+'\"'
    #     if mysql.query(sql_select):
    #         filename2 = os.path.basename(filelist[dict_result2[i][0]])
    #         sql2 = 'update similar set KgramHashim_fid = \"'+filename2+\
    #                '\", KgramHashim_number = '+str(dict_result2[i][1])+' where file_id = '+'\"'+filename+'\"'
    #         mysql.update_db(sql2)
    #     else:
    #         filename2 = os.path.basename(filelist[dict_result2[i][0]])
    #         sql2 = 'insert into similar(file_id,KgramHashim_fid,KgramHashim_number) values(\"'+filename+'\",\"'+filename2+'\",'+str(dict_result2[i][1])+')'
    #         mysql.insert_db(sql2)
...
if __name__ == '__main__':
    ...
    filelist = ['C:/Users/60917/Desktop/a.txt','C:/Users/60917/Desktop/b.txt',
                'C:/Users/60917/Desktop/c.txt','C:/Users/60917/Desktop/d.txt']

    # mysql = connect_database.Mysql()
    database = get_databasebylist(filelist)
    # dict_result = get_simhash_similarity(database)
    # print(dict_result)

    # print(get_simhash_similarity(database))
    # print(get_SparseMS(database))
    # print(get_TFIDF_cosine(database))#有问题可能是样本太小的原因,少要用四个。
    # print(get_LongSubstringSim(sql,5))#速度过慢
    #（K，windowsSize，Base）
    # print(get_KgramHashim(5,4,3,database))
    t1 = getThetxt('C:/Users/60917/Desktop/e1.txt')
    t2 = getThetxt('C:/Users/60917/Desktop/e.txt')

    t3 = getThetxt('C:/Users/60917/Desktop/t1.txt')
    t4 = getThetxt('C:/Users/60917/Desktop/t2.txt')
    print(len(t1))
    print(len(t2))
    for i in range(1,20):
        print("i:{}\tj:{}\tsim:{}".format(i,5,getKgramHashim(i,5,3,t1,t2)))
    for i in range(1,20):
        print("i:{}\tj:{}\tsim:{}".format(i, 5, getKgramHashim(i, 5, 3, t3, t4)))